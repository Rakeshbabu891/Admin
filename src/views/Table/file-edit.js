...
import { e_id } from "../Pending/FileError";
...
class FileEdit extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(`http://localhost:5000/files/${order_id}`)
      .then((response) => {
    ...
    axios
      .get(`http://localhost:5000/user/${order_id}`)
      .then((response) => {
        console.log("Response", response.data);
        this.setState({
          users: response.data,
    ...
    axios
      .get(`http://localhost:5000/quality`)
      .then((response) => {
    ...
    axios
      .get(`http://localhost:5000/density`)
      .then((response) => {
  ...
  updateOrder = () => {
    axios
      .patch(`http://localhost:5000/orders/${order_id}`, this.state.status)
      .then((response) => {
  ...
  sendemail = () => {
    ...
    if (r == true) {
      axios
        .post(`http://localhost:5000/mail/${order_id}`, this.state.status)
        .then((response) => {
    ...
    return (
      <div className="animated fadeIn">
        <Row>
          <Col xs="12" lg="4">
            <h5>Order Id</h5>
            <p style={{ backgroundColor: "white" }}>{order_id}</p>
          </Col>
          {this.state.orders
            ? this.state.orders.map((order) =>
                order.coupenApplied ? (
                  <>
                    ...
                    <Col xs="12" lg="2">
                      <h5>Coupon Amount</h5>
                      <p style={{ backgroundColor: "white" }}>
                        {order.coupen.discountAmount}
                      </p>
                    </Col>
                    <Col xs="12" lg="2">
                      <h5>Coupon Comment</h5>
                      <p style={{ backgroundColor: "white" }}>
                        {order.coupen.comment}
                      </p>
                    </Col>
                  </>
                ) : (
                  ...
                  <div className="card-body">
                    <div className="bd-example">
                      <dl className="row">
                        ...
                        <dd className="col-sm-5">
                          <select
                            onChange={this.handleChangeFor("Handled_By")}
                            value={this.state.status.Handled_By}
                            style={{ width: "113%" }}
                          >
                            {this.state.orders.map((order) => {
                              return [
                            ...
                            <option value="Aushwin(Grey Space)">
                              Aushwin(Grey Space)
                            </option>
                            ,
                            <option value="Vignesh(print 3d)">
                              Vignesh(print 3d )
                            </option>
                        ...
                        <dd className="col-sm-5">
                          <select
                            onChange={this.handleChangeFor("Print_status")}
                            value={this.state.status.Print_status}
                            style={{ width: "113%" }}
                          >
                            {this.state.orders.map((order) => {
                              return [
                            ...
                            ,
                            <option value="Print in Progress">
                              Print in Progress
                            </option>
                            ,
                            <option value="Printing Complete">
                              Printing Complete
                            </option>
                          </select>
                        </dd>
                        ...
                        <dd className="col-sm-5">
                          <select
                            onChange={this.handleChangeFor("Shipping_status")}
                            value={this.state.status.Shipping_status}
                            style={{ width: "113%" }}
                          >
                            {this.state.orders.map((order) => {
                              return (
                        ...
                        <dd className="col-sm-5">
                          <select
                            onChange={this.handleChangeFor("Courier_Services")}
                            value={this.state.status.Courier_Services}
                            style={{ width: "113%" }}
                          >
                            {this.state.orders.map((order) => {
                              return (
            ...
            <div className="card">
              ...
              <div className="card-body">
                <div className="bd-example">
                  {this.state.orders.map((order) => {
                    return (
                      <dl key={order._id} className="row">
                        <dt className="col-sm-4">Name</dt>
                        <dd className="col-sm-8">{order.name}</dd>

                        <dt className="col-sm-4">Company</dt>
                        <dd className="col-sm-8">{order.company}</dd>
                      ...
                      return (
                        <tr key={file._id}>
                          <td>
                            <a
                              href={
                                "https://tus-server-file.s3.ap-south-1.amazonaws.com/" +
                                file.fileId
                              }
                            >
                              {file.fileId}
                          ...
                          <td>
                            <select
                              onChange={this.process_change}
                              value={this.state.value}
                            >
                              <option value={file.process} selected hidden>
                                {file.processLabel}
                              </option>
                          ...
                          <td>
                            <select
                              ...
                              value={this.state.value}
                            >
                              <option value={file.material} selected hidden>
                                {file.materialLabel}
                              </option>
                              {this.state.materials.map((material) => {
                                if (
                                  this.state.process_value === material.process
                                ) {
                                  return (
                                    <option
                                      key={material._id}
                          ...
                          <td>
                            <select>
                              <option value={file.color} selected hidden>
                                {file.colorLabel}
                              </option>
                              {this.state.colors.map((color) => {
                                if (
                                  this.state.material_value === color.material
                                ) {
                                  return (
                          ...
                          <td>
                            <select>
                              ...
                              {this.state.quality.map((quality) => {
                                if (
                                  this.state.process_value == quality.process
                                ) {
                                  return (
                                    <option key={quality._id}>
        ...
        <Table>
                ...
                <div className="card-body">
                  <div className="bd-example">
                    {this.state.orders.map((order) => {
                      return (
                        <dl key={order._id} className="row">
                          {order.feedback_content !== undefined ? (
                            <dd>{order.feedback_content}</dd>
                          ) : (
                            <dd align="center">Yet to be updated</dd>
                          )}
                        </dl>
                      );
                    })}
                  ...
                  <div className="bd-example">
                    {this.state.orders.map((order) => {
                      return (
                        <dl key={order._id} className="row">
                          {order.selfpickup ? (
                            <dd align="center">Self pickup</dd>
                          ) : (
                            <dd align="center">Courier shipment</dd>
                          )}
                        </dl>
                      );
                    })}
          ...
          <Row>
            <Col sm="6"></Col>
            <Col sm="2">
              <Button block color="success" onClick={this.sendemail}>
                Save and Send{" "}
              </Button>
            </Col>
            <Col sm="2">
              <Button block color="success" onClick={this.updateOrder}>
                Save
              </Button>
            </Col>
            <Col sm="2">
